...
`assets/tiled/levels/maintenance_shaft.tmj` (the Calibration tutorial, despite its legacy filename) `assets/tiled/levels/level_01_factory.tmj` (Factory Escape) and `assets/tiled/levels/level_02_recovery.tmj` (Recovery) are the playable maps. Their external semantic tileset is `assets/tiled/semantic/semantic_layer.tsj`. `scenes/world.tscn`, `scenes/levels/level_01_factory.tscn` and `scenes/levels/level_02_recovery.tscn` wrap the imported maps; level set pieces that must return to their pre-run state on restart implement `reset_level_state()`, which `RunUnitStaticWorld.reset()` propagates; do not move authored platform geometry back into hand-maintained Godot collision nodes.
